...
// for every api request check whether user status is not frozen and device id matches the provided id.
$device_id = $get_helper['device_id'];
if ($device_id != null) {

	$sql = "SELECT * FROM tbl_users WHERE device_id = '$device_id'";
	$result = mysqli_query($mysqli, $sql);
	...
	if ($row != null) {
		if ($row['status'] == 0) {
			...
			if ($get_helper['helper_name'] == "get_cat_list") {
				...
				echo $val = str_replace('\\/', '/', json_encode($set, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT));
				die();
			} else if ($get_helper['helper_name'] == "get_q_cat_list") {

				$jsonObj = array();

				$vid_order = API_VID_ORDER_BY;

				$sql = "SELECT * FROM tbl_questions_categories";

				$result = mysqli_query($mysqli, $sql);

				while ($data = mysqli_fetch_assoc($result)) {

					$row['q_cat_id'] = $data['id'];
					$row['q_cat_name'] = $data['category_name'];
        ...
        LIMIT $limit, $page_limit";
				...
				$lan_id = $get_helper['lan_id'];
				$q_cat_id = $get_helper['catID'];
				//$q_cat_name = $get_helper['searchTerm'];
				...
				$sql = "SELECT * FROM tbl_quiz WHERE tbl_quiz.lan_id='" . $lan_id . "' AND tbl_quiz.image_type='thumbnail_block' AND tbl_quiz.status='1' AND tbl_quiz.cat_id='" . $q_cat_id . "' ORDER BY rand() DESC LIMIT $thumbnail_post";
				$result = mysqli_query($mysqli, $sql);
				...
				$sql = "SELECT * FROM tbl_quiz WHERE tbl_quiz.lan_id='" . $lan_id . "' AND tbl_quiz.cat_id='" . $q_cat_id . "' AND tbl_quiz.image_type='thumbnail_none' AND tbl_quiz.status='1'ORDER BY rand() DESC LIMIT $nonel_post";
				$result = mysqli_query($mysqli, $sql);
	...
	// if ($get_helper['helper_name'] == "user_register") {
